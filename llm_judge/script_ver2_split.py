...
DATASET_DIR = f"{BASE_DIR}/dataset/ver2"
NAIDV1_DIR  = f"{BASE_DIR}/NAIP/v1_resource/NAIDv1"
NAIDV2_DIR  = f"{BASE_DIR}/NAIP/v2_resource/NAIDv2"
OUTPUT_DIR  = "/mnt/data/son/Reviewerly/analysis_results/ver2"
...
def antonym_consistency(rot_loadings, antonym_pairs=None):
    """
    Check that loadings of antonym pairs correlate negatively (per component).
    antonym_pairs: list of (idx_a, idx_b) from get_antonym_pairs(ITEMS). Uses global ANTONYM_PAIRS if None.
    Returns per-component Pearson r between the two ends of each pair (expect r < 0).
    """
    pairs = antonym_pairs if antonym_pairs is not None else ANTONYM_PAIRS
    if not pairs:
        return {}
    k = rot_loadings.shape[1]
    results = {}
    for j in range(k):
        x = np.array([rot_loadings[a, j] for a, b in pairs])
        y = np.array([rot_loadings[b, j] for a, b in pairs])
...
def analyze_one(dataset, split, model, out_dir):
...
def item_level_correlations(dataset, split, model, out_dir):
...
def main():
    ...
    for dataset in DATASETS:
        for split in SPLITS:
            for model in MODELS:
                key = f"{dataset}_{split}_{model}"
                result = analyze_one(dataset, split, model, OUTPUT_DIR)
    ...
    # Item-level correlations (train and test)
    print("\n\n" + "=" * 80)
    print("  ITEM-LEVEL CORRELATIONS")
    print("=" * 80)
    for dataset in DATASETS:
        for split in SPLITS:
            for model in MODELS:
                item_level_correlations(dataset, split, model, OUTPUT_DIR)
