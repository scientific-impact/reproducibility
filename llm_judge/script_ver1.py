...
Applies the paper's methodology to LLM-scored abstracts from NAID v1/v2:
  ...
  2. (Optional) Average multiple runs per item per abstract (de Winter ran 4 runs:
     original, reverse order, two random orders; then averaged before analysis)
  3. (Optional) Reliability: item-mean consistency across runs (r>0.998) and
     per-abstract reliability per item (avg r≈0.27, range 0.00–0.57)
  4. Descriptive statistics (Table 2 style)
  5. PCA with Varimax rotation → 3 components
  6. Spearman correlations between component scores and impact metrics (Table 3 style)
  7. Readability analysis comparison (Table 4 style)
  8. Antonym consistency check
...
ITEMS = ITEMS_PROMPT_ORDER_VER1
MARKER_ITEMS = get_marker_indices(ITEMS)
ANTONYM_PAIRS = get_antonym_pairs(ITEMS)

BASE_DIR   = "/mnt/data/son/Reviewerly"
DATASET_DIR = f"{BASE_DIR}/dataset/ver1"
NAIDV1_DIR  = f"{BASE_DIR}/NAIP/v1_resource/NAIDv1"
NAIDV2_DIR  = f"{BASE_DIR}/NAIP/v2_resource/NAIDv2"
OUTPUT_DIR  = "/mnt/data/son/Reviewerly/analysis_results/ver1"

MODELS  = [ "GPT", "Deepseek", "Llama-3.1", "Llama-3.3"]
DATASETS = ["naidv1", "naidv2"]
SPLITS   = ["train", "test"]

# Multiple runs (de Winter 2024): set to list of filenames under each (split, dataset, model)
# to load several runs and average scores per item per abstract before analysis.
# Example: ["llm_scores.json", "llm_scores_reverse.json", "llm_scores_r1.json", "llm_scores_r2.json"]
# If None, only the single file "llm_scores.json" is used.
MULTIPLE_RUN_FILES = None
