...
def parse_args() -> argparse.Namespace:
    ...
    # p.add_argument("--test_data_path", type=str, help="CSV with the evaluation set", default="/v1_resource/NAIDv1/NAID_test_extrainfo.csv")
    p.add_argument("--test_data_path", type=str, help="CSV with the evaluation set", default="/v2_resource/NAIDv2/NAIDv2-test-with-cites.csv")

    p.add_argument("--gt_field", type=str, default="TNCSI_SP", help="Ground-truth field name for regression / ranking metrics")

    # p.add_argument("--test_data_path", type=str, help="CSV with the evaluation set", default="/v2_resource/NAIDv2/NAIDv2-test.csv")
    # p.add_argument("--gt_field", type=str, default="RTS", help="Ground-truth field name for regression / ranking metrics")
...
def main():
    ...
    if "id" in df_val.columns:
        ...
        available_meta_cols = ["id"]
        for col in ["pub_year", "cluster_cat"]:
            if col in df_val.columns:
                available_meta_cols.append(col)
        
        if len(available_meta_cols) > 1:  # More than just "id"
            meta = df_val[available_meta_cols].copy()
            meta["id"] = meta["id"].astype(str)
            merged = df_out.merge(meta, on="id", how="left", suffixes=("", "_m"))

            need_fallback = False
            for col in available_meta_cols[1:]:  # Skip "id"
                if f"{col}_m" in merged.columns:
                    nan_ratio = merged[f"{col}_m"].isna().mean()
                    if nan_ratio > 0.3:
                        need_fallback = True
                        break
